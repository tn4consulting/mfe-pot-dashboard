...
- **CI** (`.github/workflows/ci.yml`): lint/test/build (including `benefit-aggregation-bff`'s separately-run `eslint:lint` target), then builds both images, spins up an ephemeral `kind` cluster, `helm install`s this chart, and curls the Ingress-routed hostname to confirm it serves. The `/api/overview` smoke check deliberately omits the `sub` query param (asserts a 400, not a 404) rather than exercising the real fan-out, since this workflow doesn't also deploy job-bank/employment-insurance/client-profile-service. No `README.md` yet — still open work tracked in `../TODO.md`.
